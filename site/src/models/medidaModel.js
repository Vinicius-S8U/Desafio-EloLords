...
function pct_acertos() {
    ...
    if (process.env.AMBIENTE_PROCESSO == "producao") {
        instrucaoSql = `select count(id_questionario) as 'qtd_pessoas',count( case when acertos = 9 then 1 else null end) as 'acertos9',count( case when acertos = 9 then 1 else null end) as 'acertos9',count(case when acertos = 8 then 1 else null end) as 'acertos8',
        ...
        count(case when acertos = 0 then 1 else null end) as 'acertos0',count(case when quest10 = 1 then 1 else null end) as 'muito',
